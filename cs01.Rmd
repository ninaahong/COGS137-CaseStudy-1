...
In order to begin our analysis, we imported our raw data from our data/ folder.
...
To begin our data wrangling, we took the imported data, in the form of a table, and modified the columns and transformed its data to reorganize it for better structure. There are three datasets for each fluid type, blood, oral, and breath, we reformatted the entries in the `Treatment` column to make them easier to read. We also turned all the columns into snake case. For all the compounds in each dataset, we renamed the column names.
