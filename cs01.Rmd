...
df_long <- WB %>%
  pivot_longer(cols = c(THC, `11-OH-THC`, `THC-COOH`), names_to = "Compound", values_to = "Level")

# Plot THC and metabolites over time for each matrix
ggplot(df_long, aes(x = time.from.start, y = Level, color = Compound)) +
  geom_point(size = 1) +
